...
const empty: CForm = { name: "", phone: "", email: "", address: "", city: "", notes: "" };
  ...
  const upsert = useMutation({
    mutationFn: async (c: CForm) => {
      ...
      if (c.id) {
        ...
        const { error } = await supabase.from("customers").insert({ ...payload, business_id: business.id });
        if (error) throw error;
      }
    },
    onSuccess: () => {
  ...
  return (
    <div className="container mx-auto p-6 lg:p-10 max-w-7xl">
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        {customers.length === 0 ? (
          <div className="p-12 text-center text-muted-foreground">{t("noCustomers")}</div>
        ) : (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{form.id ? t("edit") : t("newCustomer")}</DialogTitle>
          </DialogHeader>
